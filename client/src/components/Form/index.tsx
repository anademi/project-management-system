...
export const Form = () => {
  const { tasks, boards, users, createTask, updateTask } = useStore();
  const {
    setIsModalOpen,
    taskData,
    setTaskData,
  ...
  const currentTask =
    taskData.id && tasks.find((task) => task.id === taskData.id);
        ...
        {isBoardPage ? (
          <label>
            ...
            <select
              ...
              disabled
            >
              <option value={idParams}>{getBoardName(idParams)}</option>
            </select>
          </label>
        ) : (
          <label>
        ...
        <label>
          ...
          >
            {PRIORITIES.map((element) => {
              return (
                <option key={element} value={element as string}>
                  {element}
                </option>
              );
            })}
